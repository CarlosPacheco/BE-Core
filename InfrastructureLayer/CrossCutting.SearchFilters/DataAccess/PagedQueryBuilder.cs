...
namespace CrossCutting.SearchFilters.DataAccess
{
    ...
    {
        private static readonly string SqlTotalCountSelectClauseValue = @"COUNT(1) TotalCount";
        private static readonly Regex SqlSelectClauseRegex = new Regex(@"(SELECT)(.*?)( FROM)", RegexOptions.Compiled | RegexOptions.IgnoreCase | RegexOptions.Singleline);
