...
namespace Dapper
{
    ...
    public static partial class SimpleCRUD
    {
        ...
        public static TKey Insert<TKey, TEntity>(this IDbConnection connection, TEntity entityToInsert, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            ...
            {
                Guid guidvalue = (Guid)idProps.First().GetValue(entityToInsert, null);
            ...
            {
                sb.Append((_dialect == Dialect.PostgreSQL ? " " : ";") + _getIdentitySql);
            }
            else
            {
                keyHasPredefinedValue = true;
        ...
        public static bool Update<TEntity>(this IDbConnection connection, TEntity entityToUpdate, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            ...
            return connection.Execute(masterSb.ToString(), entityToUpdate, transaction, commandTimeout) > 0;
        }
        ...
        private static void BuildWhere<TEntity>(StringBuilder sb, IEnumerable<PropertyInfo> idProps, object whereConditions = null)
        {
            ...
            for (int i = 0; i < propertyInfos.Count(); i++)
            {
                ...
                PropertyInfo[] sourceProperties = GetScaffoldableProperties<TEntity>().ToArray();
        ...
        private static bool IsEditable(PropertyInfo pi)
        {
            ...
            {
                EditableAttribute write = attributes.FirstOrDefault(x => x.GetType().Name == typeof(EditableAttribute).Name) as EditableAttribute;
                if (write != null)
                {
        ...
        private static bool IsReadOnly(PropertyInfo pi)
        {
            ...
            if (attributes.Length > 0)
            {
                ReadOnlyAttribute write = attributes.FirstOrDefault(x => x.GetType().Name == typeof(ReadOnlyAttribute).Name) as ReadOnlyAttribute;
                if (write != null)
                {
        ...
        {
            IEnumerable<PropertyInfo> updateableProperties = GetScaffoldableProperties<T>();
            //remove ones with ID
        ...
        public class TableNameResolver : ITableNameResolver
        {
            public virtual string ResolveTableName(Type type)
            {
                ...
                TableAttribute tableattr = type.GetCustomAttributes(true).SingleOrDefault(attr => attr.GetType().Name == typeof(TableAttribute).Name) as TableAttribute;
                if (tableattr != null)
                {
        ...
        public class ColumnNameResolver : IColumnNameResolver
        {
            public virtual string ResolveColumnName(PropertyInfo propertyInfo)
            {
                ...
                ColumnAttribute columnattr = propertyInfo.GetCustomAttributes(true).SingleOrDefault(attr => attr.GetType().Name == typeof(ColumnAttribute).Name) as ColumnAttribute;
                if (columnattr != null)
                {
